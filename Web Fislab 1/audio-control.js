// Audio Control System for FISLAB Website
// This file provides background music functionality with speaker on/off toggle
// Enhanced for continuous playback across page navigation
    ...
    setupCrossPageCommunication() {
        ...
        window.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'AUDIO_STATE_UPDATE') {
                // Update local state based on message from other page
                const { isPlaying, currentTime, volume } = event.data;
        ...
        // Broadcast audio state changes to other pages
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Create and initialize audio controller
    window.fislabAudio = new AudioController();
});
